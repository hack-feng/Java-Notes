...
`DelayQueue` 是 JUC 包(`java.util.concurrent)`为我们提供的延迟队列，用于实现延时任务比如订单下单 15 分钟未支付直接取消。它是 `BlockingQueue` 的一种，底层是一个基于 `PriorityQueue` 实现的一个无界队列，是线程安全的。关于`PriorityQueue`可以参考笔者编写的这篇文章：PriorityQueue 源码分析 。

![笑小枫-www.xiaoxiaofeng.com](https://image.xiaoxiaofeng.site/spider/2024/11/21/xxf-1732153718080.png) BlockingQueue 的实现类
...
![笑小枫-www.xiaoxiaofeng.com](https://image.xiaoxiaofeng.site/spider/2024/11/21/xxf-1732153718653.png) 延迟任务
...
![笑小枫-www.xiaoxiaofeng.com](https://image.xiaoxiaofeng.site/spider/2024/11/21/xxf-1732153719002.png) DelayQueue类图
...
> 
>  *  [图文讲解 AQS ，一起看看 AQS 的源码……(图文较长)][AQS _ AQS]
>  *  [AQS 都看完了，Condition 原理可不能少！][AQS _Condition]

1、首先， 3 个线程会尝试获取可重入锁 `lock`,假设我们现在有 3 个线程分别是 t1、t2、t3,随后 t1 得到了锁，而 t2、t3 没有抢到锁，故将这两个线程存入等待队列中。

![笑小枫-www.xiaoxiaofeng.com](https://image.xiaoxiaofeng.site/spider/2024/11/21/xxf-1732153719444.png)
...
![笑小枫-www.xiaoxiaofeng.com](https://image.xiaoxiaofeng.site/spider/2024/11/21/xxf-1732153719779.png)

注意，调用 `await` 之后 t1 就会释放 `lcok` 锁，假如 `DelayQueue` 持续为空，那么 t2、t3 也会像 t1 一样执行相同的逻辑并进入 `conditionWaiter` 队列中。

![笑小枫-www.xiaoxiaofeng.com](https://image.xiaoxiaofeng.site/spider/2024/11/21/xxf-1732153720128.png)
...
[BlockingQueue]: https://oss.javaguide.cn/github/javaguide/java/collection/blocking-queue-hierarchy.png
[delayed-task.png]: https://oss.javaguide.cn/github/javaguide/java/collection/delayed-task.png
[DelayQueue]: https://oss.javaguide.cn/github/javaguide/java/collection/delayqueue-class-diagram.png
[AQS _ AQS]: https://xie.infoq.cn/article/5a3cc0b709012d40cb9f41986
[AQS _Condition]: https://xie.infoq.cn/article/0223d5e5f19726b36b084b10d
[delayqueue-take-0.png]: https://oss.javaguide.cn/github/javaguide/java/collection/delayqueue-take-0.png
[delayqueue-take-1.png]: https://oss.javaguide.cn/github/javaguide/java/collection/delayqueue-take-1.png
[delayqueue-take-2.png]: https://oss.javaguide.cn/github/javaguide/java/collection/delayqueue-take-2.png
[https_blog.csdn.net_every_day_article_details_113810985]: https://blog.csdn.net/every__day/article/details/113810985
