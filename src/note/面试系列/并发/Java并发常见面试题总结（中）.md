...
![笑小枫-www.xiaoxiaofeng.com](https://image.xiaoxiaofeng.site/spider/2024/11/21/xxf-1732154495130.png) 中进行读取
...
![JUC原子类概览](https://image.xiaoxiaofeng.site/blog/2024/11/21/xxf-20241121100341.png?xxfjava)
...
关于 `Unsafe` 类的详细介绍可以看这篇文章：[Java 魔法类 Unsafe 详解 - JavaGuide - 2022][Java _ Unsafe _ - JavaGuide - 2022] 。

### Java 中 CAS 是如何实现的？ ###

在 Java 中，实现 CAS（Compare-And-Swap, 比较并交换）操作的一个关键类是`Unsafe`。

`Unsafe`类位于`sun.misc`包下，是一个提供低级别、不安全操作的类。由于其强大的功能和潜在的危险性，它通常用于 JVM 内部或一些需要极高性能和底层访问的库中，而不推荐普通开发者在应用程序中使用。关于 `Unsafe`类的详细介绍，可以阅读这篇文章：📌[Java 魔法类 Unsafe 详解][Java _ Unsafe _ - JavaGuide - 2022]。
...
![JUC原子类概览](https://image.xiaoxiaofeng.site/blog/2024/11/21/xxf-20241121100404.png?xxfjava)

关于这些 Atomic 原子类的介绍和使用，可以阅读这篇文章：[Atomic 原子类总结][Atomic]。
...
![笑小枫-www.xiaoxiaofeng.com](https://image.xiaoxiaofeng.site/spider/2024/11/21/xxf-1732154495543.png) synchronized关键字原理
...
![笑小枫-www.xiaoxiaofeng.com](https://image.xiaoxiaofeng.site/spider/2024/11/21/xxf-1732154496063.png) 

对象锁的的拥有者线程才可以执行 `monitorexit` 指令来释放锁。在执行 `monitorexit` 指令后，将锁计数器设为 0，表明锁被释放，其他线程可以尝试获取锁。

![笑小枫-www.xiaoxiaofeng.com](https://image.xiaoxiaofeng.site/spider/2024/11/21/xxf-1732154496420.png) 
...
![synchronized关键字原理](https://image.xiaoxiaofeng.site/blog/2024/11/21/xxf-20241121100444.png?xxfjava)
...
相关推荐：[Java 锁与线程的那些事 - 有赞技术团队][Java _ -] 。
...
`synchronized` 锁升级是一个比较复杂的过程，面试也很少问到，如果你想要详细了解的话，可以看看这篇文章：[浅析 synchronized 锁升级的原理与实现][synchronized 2]。
...
![笑小枫-www.xiaoxiaofeng.com](https://image.xiaoxiaofeng.site/spider/2024/11/21/xxf-1732154497468.png)
...
从上面的内容可以看出， `ReentrantLock` 的底层就是由 AQS 来实现的。关于 AQS 的相关内容推荐阅读 [AQS 详解][AQS] 这篇文章。
...
![笑小枫-www.xiaoxiaofeng.com](https://image.xiaoxiaofeng.site/spider/2024/11/21/xxf-1732154497931.png)
...
读写锁的源码分析，推荐阅读 [聊聊 Java 的几把 JVM 级锁 - 阿里巴巴中间件][Java _ JVM _ -] 这篇文章，写的很不错。
...
另外，`StampedLock` 性能虽好，但使用起来相对比较麻烦，一旦使用不当，就会出现生产问题。强烈建议你在使用`StampedLock` 之前，看看 [StampedLock 官方文档中的案例][StampedLock]。
 ...
 *  [AQS 详解][AQS]
 *  [StampedLock 底层原理分析][StampedLock 1]
...
[JMM_Java 1]: https://oss.javaguide.cn/github/javaguide/java/concurrent/jmm2.png
[JUC]: https://oss.javaguide.cn/github/javaguide/java/JUC原子类概览-20230814005211968.png
[Java _ Unsafe _ - JavaGuide - 2022]: https://javaguide.cn/java/basis/unsafe.html
[JUC 1]: https://oss.javaguide.cn/github/javaguide/java/JUC原子类概览.png
[Atomic]: https://javaguide.cn/java/concurrent/atomic-classes.html
[synchronized]: https://oss.javaguide.cn/github/javaguide/java/concurrent/synchronized-principle.png
...
[monitorenter]: https://oss.javaguide.cn/github/javaguide/java/concurrent/synchronized-get-lock-code-block.png
[monitorexit]: https://oss.javaguide.cn/github/javaguide/java/concurrent/synchronized-release-lock-block.png
[synchronized 1]: https://oss.javaguide.cn/github/javaguide/synchronized关键字原理2.png
[Java _ -]: https://tech.youzan.com/javasuo-yu-xian-cheng-de-na-xie-shi/
[synchronized 2]: https://www.cnblogs.com/star95/p/17542850.html
[reentrantlock-class-diagram.png]: https://oss.javaguide.cn/github/javaguide/java/concurrent/reentrantlock-class-diagram.png
[AQS]: https://javaguide.cn/java/concurrent/aqs.html
[reentrantreadwritelock-class-diagram.png]: https://oss.javaguide.cn/github/javaguide/java/concurrent/reentrantreadwritelock-class-diagram.png
[Java _ JVM _ -]: https://mp.weixin.qq.com/s/h3VIUyH9L0v14MrQJiiDbw
[StampedLock]: https://docs.oracle.com/javase/8/docs/api/java/util/concurrent/locks/StampedLock.html
[StampedLock 1]: https://segmentfault.com/a/1190000015808032
